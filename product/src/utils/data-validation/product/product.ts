 ...
 *
 * @property {"Simple Product" | "Variable Product" | "Customized Product"} value - The type of product configuration.
 */
export const ProductConfigurationTypeEnum = z.enum(
  ["Simple Product", "Variable Product", "Customized Product"],
  {
    errorMap: () => ({
      message:
        "Product configuration type must be 'Simple Product', 'Variable Product', or 'Customized Product'",
    }),
  }
);
...
export const StockStatusEnum = z.enum(
  ...
  {
    errorMap: () => ({
      message: "Stock status must be 'In stock', 'Out of stock', or 'On backorder'",
    }),
  }
);
...
export const ProductTieredPriceInputSchema = z.object({
  id: z.string().uuid().optional(),
  minQuantity: z.number().int().positive("Min quantity must be a positive integer"),
  maxQuantity: z.number().int().positive("Max quantity must be a positive integer"),
  quantityUnit: z.string().min(1, "Quantity unit cannot be empty").trim(),
  fixedPrice: z.number().positive("Fixed price must be a positive number").optional().nullable(),
  percentageDiscount: z.number().min(0).max(100).optional().nullable(),
  productPriceId: z.string().uuid("Invalid product price ID format").optional(),
});
...
export const ProductPriceInputSchema = z.object({
  ...
  pricingType: z.enum(["Fixed", "Percentage"], {
    errorMap: () => ({ message: "Pricing type must be 'Fixed' or 'Percentage'" }),
  }),
  tieredPrices: z.array(ProductTieredPriceInputSchema).optional(),
  productId: z.string().uuid("Invalid product ID format").optional(),
  productVariationId: z.string().uuid("Invalid product variation ID format").optional(),
});
...
export const ProductVariationAttributeValueInputSchema = z.object({
  ...
  attributeId: z.string().uuid("Invalid variation attribute ID format"),
 ...
 * 3. Validates `productDeliveryType` as an optional array of `ProductDeliveryTypeEnum`.
 * 4. Validates `minQuantity`, `defaultQuantity`, `maxQuantity`, `quantityStep` as optional positive integers.
 * 5. Ensures `regularPrice` is a positive number.
 * 6. Validates `salePrice` as an optional positive number.
 * 7. Validates `salePriceStartAt` and `salePriceEndAt` as optional datetime strings.
 * 8. Validates `tierPricingInfoId` as an optional UUID.
 ...
 * 14. Validates `productId` as a UUID.
 ...
 * 19. Validates `shippingClassId`, `taxStatusId`, `taxClassId` as optional UUIDs.
...
export const ProductVariationInputSchema = z.object({
  ...
  minQuantity: z.number().int().positive("Min quantity must be a positive integer").optional().nullable(),
  defaultQuantity: z.number().int().positive("Default quantity must be a positive integer").optional().nullable(),
  maxQuantity: z.number().int().positive("Max quantity must be a positive integer").optional().nullable(),
  quantityStep: z.number().int().positive("Quantity step must be a positive integer").optional(),
  ...
  tierPricingInfoId: z.string().uuid("Invalid tier pricing info ID format").optional().nullable(),
  ...
  length: z.number().positive("Length must be a positive number").optional().nullable(),
  ...
  attributeValues: z.array(ProductVariationAttributeValueInputSchema).optional(),
  warrantyDigit: z.number().int().positive("Warranty digit must be a positive integer").optional().nullable(),
  defaultWarrantyPeriod: WarrantyPeriodEnum.optional().nullable(),
  warrantyPolicy: z.string().min(1, "Warranty policy cannot be empty").optional().nullable(),
  shippingClassId: z.string().uuid("Invalid shipping class ID format").optional().nullable(),
  taxStatusId: z.string().uuid("Invalid tax status ID format").optional().nullable(),
  taxClassId: z.string().uuid("Invalid tax class ID format").optional().nullable(),
  description: z.string().min(1, "Description cannot be empty").optional().nullable(),
  images: z.array(z.string().uuid("Invalid image ID format")).optional().nullable(),
  videos: z.array(z.string().uuid("Invalid video ID format")).optional().nullable(),
 ...
 * 8. Validates `brandId`, `tagIds`, `categoryId`, `subCategoryIds` as optional UUIDs or arrays of UUIDs.
...
export const createProductSchema = z.object({
  ...
  createdBy: z.string().uuid("Invalid created by ID format"),
  defaultImage: z.string().uuid("Invalid default image ID format").optional().nullable(),
  images: z.array(z.string().uuid("Invalid image ID format")).optional().nullable(),
  videos: z.array(z.string().uuid("Invalid video ID format")).optional().nullable(),
  defaultMainDescription: z.string().min(1, "Main description cannot be empty").trim(),
  defaultShortDescription: z.string().min(1, "Short description cannot be empty").optional().nullable(),
  defaultTags: z.array(z.string().min(1, "Tag cannot be empty")).optional().nullable(),
  customBadge: z.string().min(1, "Custom badge cannot be empty").optional().nullable(),
  purchaseNote: z.string().min(1, "Purchase note cannot be empty").optional().nullable(),
  brandId: z.string().uuid("Invalid brand ID format").optional().nullable(),
  tagIds: z.array(z.string().uuid("Invalid tag ID format")).optional().nullable(),
  categoryId: z.string().uuid("Invalid category ID format"),
  subCategoryIds: z.array(z.string().uuid("Invalid sub-category ID format")).optional().nullable(),
  warrantyDigit: z.number().int().positive("Warranty digit must be a positive integer").optional().nullable(),
  defaultWarrantyPeriod: WarrantyPeriodEnum.optional().nullable(),
  warrantyPolicy: z.string().min(1, "Warranty policy cannot be empty").optional().nullable(),
  regularPrice: z.number().positive("Regular price must be a positive number"),
  ...
  saleQuantity: z.number().int().positive("Sale quantity must be a positive integer").optional().nullable(),
  saleQuantityUnit: z.string().min(1, "Sale quantity unit cannot be empty").trim(),
  taxStatusId: z.string().uuid("Invalid tax status ID format").optional().nullable(),
  taxClassId: z.string().uuid("Invalid tax class ID format").optional().nullable(),
  minQuantity: z.number().int().positive("Min quantity must be a positive integer").optional().nullable(),
  defaultQuantity: z.number().int().positive("Default quantity must be a positive integer").optional().nullable(),
  ...
  stockQuantity: z.number().int().positive("Stock quantity must be a positive integer").optional().nullable(),
  allowBackOrders: BackOrderOptionEnum.optional().nullable(),
  lowStockThresHold: z.number().int().positive("Low stock threshold must be a positive integer").optional().nullable(),
  stockStatus: StockStatusEnum.optional().nullable(),
  soldIndividually: z.boolean().optional(),
  initialNumberInStock: z.string().min(1, "Initial number in stock cannot be empty").optional().nullable(),
  weightUnit: WeightUnitEnum.optional().nullable(),
  weight: z.number().positive("Weight must be a positive number").optional().nullable(),
  dimensionUnit: DimensionUnitEnum.optional().nullable(),
  length: z.number().positive("Length must be a positive number").optional().nullable(),
  ...
  height: z.number().positive("Height must be a positive number").optional().nullable(),
  shippingClassId: z.string().uuid("Invalid shipping class ID format").optional().nullable(),
  upsellIds: z.array(z.string().uuid("Invalid upsell ID format")).optional().nullable(),
  crossSellIds: z.array(z.string().uuid("Invalid cross-sell ID format")).optional().nullable(),
...
export const updateProductSchema = z.object({
  ...
  name: z.string().min(3, "Product name must be at least 3 characters").trim().optional().nullable(),
  slug: z.string().min(3, "Product slug must be at least 3 characters").trim().optional().nullable(),
  sku: z.string().min(1, "SKU cannot be empty").optional().nullable(),
  model: z.string().min(1, "Model cannot be empty").optional().nullable(),
  defaultImage: z.string().uuid("Invalid default image ID format").optional().nullable(),
  images: z.array(z.string().uuid("Invalid image ID format")).optional().nullable(),
  videos: z.array(z.string().uuid("Invalid video ID format")).optional().nullable(),
  defaultMainDescription: z.string().min(1, "Main description cannot be empty").trim().optional().nullable(),
  defaultShortDescription: z.string().min(1, "Short description cannot be empty").optional().nullable(),
  defaultTags: z.array(z.string().min(1, "Tag cannot be empty")).optional().nullable(),
  customBadge: z.string().min(1, "Custom badge cannot be empty").optional().nullable(),
  purchaseNote: z.string().min(1, "Purchase note cannot be empty").optional().nullable(),
  ...
  defaultWarrantyPeriod: WarrantyPeriodEnum.optional().nullable(),
  warrantyPolicy: z.string().min(1, "Warranty policy cannot be empty").optional().nullable(),
  regularPrice: z.number().positive("Regular price must be a positive number").optional().nullable(),
  salePrice: z.number().positive("Sale price must be a positive number").optional().nullable(),
  salePriceStartAt: z.string().datetime().optional().nullable(),
  salePriceEndAt: z.string().datetime().optional().nullable(),
  tierPricingInfo: ProductPriceInputSchema.optional().nullable(),
  saleQuantity: z.number().int().positive("Sale quantity must be a positive integer").optional().nullable(),
  saleQuantityUnit: z.string().min(1, "Sale quantity unit cannot be empty").trim().optional().nullable(),
  taxStatusId: z.string().uuid("Invalid tax status ID format").optional().nullable(),
  taxClassId: z.string().uuid("Invalid tax class ID format").optional().nullable(),
  minQuantity: z.number().int().positive("Min quantity must be a positive integer").optional().nullable(),
  defaultQuantity: z.number().int().positive("Default quantity must be a positive integer").optional().nullable(),
  maxQuantity: z.number().int().positive("Max quantity must be a positive integer").optional().nullable(),
  quantityStep: z.number().int().positive("Quantity step must be a positive integer").optional().nullable(),
  manageStock: z.boolean().optional().nullable(),
  stockQuantity: z.number().int().positive("Stock quantity must be a positive integer").optional().nullable(),
  allowBackOrders: BackOrderOptionEnum.optional().nullable(),
  lowStockThresHold: z.number().int().positive("Low stock threshold must be a positive integer").optional().nullable(),
